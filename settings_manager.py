...
class SettingsManager:
    ...
    def load(self, config_file=None):
        ...
        if config_file.exists():
            with open(config_file, "r") as f:
                ...
                # Remove settings from user_settings if they are the same as the defaults
                for key in list(self.user_settings):  # Use list to avoid RuntimeError for changing dict size during iteration
                    if key in self.settings and self.user_settings[key] == self.settings[key]:
                        self.user_settings.pop(key)
                self.settings.update(self.user_settings)  # Merge remaining user settings over defaults
    ...
    def set(self, key, value):
        ...
        if key not in default_settings or default_settings[key] != value:
            ...
            # If the value is the same as the default, remove it from user_settings
